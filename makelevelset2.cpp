...
void make_level_set2(const std::vector<Vec2ui> &edge, const std::vector<Vec2d> &x,
                     const Vec2d &origin, double dx, int nx, int ny, Array2d &phi)
{
    phi.resize(nx, ny);
    ...
    for(int j=0; j<ny; ++j)
    {
        ...
        for(int i=0; i<nx; ++i)
        {
            ...
            if(total_count%2==1)
                phi(i,j)=-phi(i,j);// if parity of intersections so far is odd,  we are inside the mesh
